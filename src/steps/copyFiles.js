...
const copyFiles = async (type, name) => {
  ...
  spinner.succeed(chalk.green.bold(`Success! Created project folder ${name}`)).start();
  try {
    ...
    console.log(
      chalk.red.bold(`Please check the console output and report it at 'https://github.com/nos/create-nos-dapp/issues'`)
    );
  }
};
